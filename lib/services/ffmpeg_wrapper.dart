...
class FFmpegWrapper {
  ...
  Future<int?> getVideoDuration(String path) async {
    try {
      ...
      if (information != null) {
        final durationStr = information.getDuration(); // String?
        if (durationStr != null && durationStr.isNotEmpty) {
          // Convertir a double (la duración viene en segundos como string, ej: "10.5")
  ...
  Future<bool> processVideo({
    ...
    try {
      ...
      // ✅ CORRECCIÓN: usar parámetros con nombre para los callbacks
      _currentSession = await FFmpegKit.executeWithArguments(
        arguments,
        completeCallback: (session) {
          // Manejar con .then para evitar async en el callback (aunque async void es aceptable)
